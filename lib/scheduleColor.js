export function getScheduleColor(schedule) {
  if (!schedule) return "#a8a29c";
  const s = schedule.toLowerCase();

  if (s.includes("every load") || s.includes("as needed")) return "#a8a29c";
  ...
  if (monthsMatch) {
    ...
    if (n <= 5)  return "#4ade80"; // teal   (3–5 months)
  ...
  if (s.includes("3–6 month") || s.includes("2–4")) return "#4ade80";
